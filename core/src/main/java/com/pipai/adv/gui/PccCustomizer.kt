...
class PccCustomizer(pcc: List<PccMetadata>,
                    ...
                    private val skin: Skin) : ScrollPane(Table(), skin) {
    ...
    init {
        ...
        table.left().top()
        table.add(verticalGroup).pad(PADDING)
        rebuildList()
    }
    ...
    private fun buildListElement(index: Int): Actor {
        ...
        listTable.add(deleteButton)
                .width(BUTTON_SIZE).minWidth(BUTTON_SIZE)
        listTable.add(shiftDownButton)
                .width(BUTTON_SIZE).minWidth(BUTTON_SIZE).padLeft(4f)
        ...
        listTable.add(colorBox1)
                .width(BUTTON_SIZE).minWidth(BUTTON_SIZE).height(BUTTON_SIZE).maxHeight(BUTTON_SIZE).padLeft(4f)
