...
fun npcFromStats(stats: UnitStats, weapon: InventoryItem.WeaponInstance?): Npc {
    val schema = UnitSchema("Test Unit", stats, Resistances(), 0, listOf())
...
fun getSchemaList(): UnitSchemaIndex {
    ...
    schemaList.addSchema("Human", UnitStats(20, 10, 10, 10, 10, 10, 10, 0, 5), Resistances(), 0, listOf(),
            EnvObjTilesetMetadata.NONE)
    schemaList.addSchema("Slime", UnitStats(15, 20, 5, 5, 20, 15, 5, 0, 3), Resistances(), 0, listOf(),
            EnvObjTilesetMetadata.NONE)
    return schemaList
}
